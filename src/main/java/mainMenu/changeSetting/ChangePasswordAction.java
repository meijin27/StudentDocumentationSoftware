...
public class ChangePasswordAction extends Action {
	...
	public String execute(
		...
		if (ValidationUtil.isNullOrEmpty(oldPassword)) {
			request.setAttribute("oldPasswordError", "現在のパスワードの入力は必須です");
		} else if (ValidationUtil.areValidLengths(32, oldPassword)) {
			request.setAttribute("oldPasswordError", "パスワードが一致しません。再度入力してください。");
		} else if (!Pattern.matches("^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])[a-zA-Z0-9]{8,}$", oldPassword)) {
			request.setAttribute("oldPasswordError", "パスワードが一致しません。再度入力してください。");
		}

		// 新パスワードの入力チェック
		// 未入力及び不一致はエラー処理		
		if (ValidationUtil.isNullOrEmpty(newPassword, passwordCheck)) {
			request.setAttribute("passwordError", "新パスワードの入力は必須です");
		...
		} else if (!Pattern.matches("^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])[a-zA-Z0-9]{8,}$", newPassword)) {
